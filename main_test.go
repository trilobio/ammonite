...
func TestLabwareApi(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/labware", bytes.NewReader(m))
	resp := httptest.NewRecorder()
	app.Router.ServeHTTP(resp, req)
	...
	req = httptest.NewRequest("GET", "/api/labware/apiPlate", nil)
	resp = httptest.NewRecorder()
	app.Router.ServeHTTP(resp, req)
	var labware Labware
	reqBody, err = ioutil.ReadAll(resp.Body)
	if err != nil {
		t.Errorf("Reading /api/labware/apiPlate should succeed. Got error: %s", err)
	}
	...
	req = httptest.NewRequest("DELETE", "/api/labware/apiPlate", nil)
	resp = httptest.NewRecorder()
	app.Router.ServeHTTP(resp, req)
	if strings.TrimSpace(resp.Body.String()) != success {
		t.Errorf("Unexpected response. Expected: " + success + "\nGot: " + resp.Body.String())
...
func TestDeckApi(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/deck", bytes.NewReader(m))
	resp := httptest.NewRecorder()
	app.Router.ServeHTTP(resp, req)
	...
	req = httptest.NewRequest("GET", "/api/deck/defaultDeck", nil)
	resp = httptest.NewRecorder()
	app.Router.ServeHTTP(resp, req)
	var deck Deck
	reqBody, err = ioutil.ReadAll(resp.Body)
	if err != nil {
		t.Errorf("Reading /api/deck/defaultDeck should succeed. Got error: %s", err)
	}
	...
	req = httptest.NewRequest("DELETE", "/api/deck/defaultDeck", nil)
	resp = httptest.NewRecorder()
	app.Router.ServeHTTP(resp, req)
	if strings.TrimSpace(resp.Body.String()) != success {
		t.Errorf("Unexpected response. Expected: " + success + "\nGot: " + resp.Body.String())
